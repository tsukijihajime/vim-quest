...
function resolveMotion(
  ...
  switch (key) {
    ...
    // 注意: オペレータと合成するときは wordForwardForOperator を使う。
    // 下の applyNormalKey を参照。
    case 'b':
...
`applyOperator` に `c` の 2 分岐を足す。

**注意: `applyOperator` の実際のシグネチャは 3 引数である。**
Task 6 の当初案にあった `stopAtLineEnd` 引数とクランプ処理は、
`dw` の行末特例の判定が誤っていたため撤去済みで、その役目は
`motions.ts` の `wordForwardForOperator` が引き取っている。
4 引数版に戻してはいけない。
...
function applyOperator(
  ...
  if (start.row === end.row && start.col === end.col) return reset(state)
  if (op === 'd') return applyCharwiseDelete(state, start, end)
  if (op === 'c') return applyCharwiseChange(state, start, end)
  return reset(state)
}
```
...
`applyNormalKey` に `cw` の特例を入れる。

**挿入位置が決定的に重要である。** Task 6 の実装には、オペレータが立っている
ときの `w` / `W` を `wordForwardForOperator` に振り替える分岐が既に入っている。
`cw` の特例は**その分岐より前**に置かなければならない。後ろに置くと `cw` が
w/W の振り替えに先に捕まり、`changeWordEnd` に到達しない。

また `applyOperator` は 3 引数である（Task 6 で `stopAtLineEnd` 引数は撤去された）。
ここでは既存の `finishMotion` を使う。
...
```ts
  ...
  // ここから下は Task 6 で既に書かれている。触らずそのまま残す
...
export function starsFor(keystrokes: number, par: number): Stars {
  ...
  // 仕様は「keystrokes <= par * 1.5」。keystrokes は常に整数なので
  // floor を取っても同値になる。ceil にすると par が奇数のとき境界が
  // 1 つ緩み、par=7 / keystrokes=11 が ☆2 になってしまう（正しくは ☆1）。
